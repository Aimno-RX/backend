...
async def extract_knowledge_graph_from_file(
    ...
    try:
        ...
        # elif params.source_type == 'youtube' and params.source_url:  # 注释：不使用YouTube功能
...
@app.get('/statistics')
async def get_statistics(credentials: Neo4jCredentials = Depends(get_neo4j_credentials)):
    """
    获取知识图谱统计信息
    包括：总文档数、总分块数、总实体数、总关系数、问答会话数
    """
    try:
        start = time.time()
        graph = Neo4jGraph(
        ...
        # 统计查询
        total_documents = graph.query("MATCH (d:Document) RETURN count(d) as count")[0]['count']
        total_chunks = graph.query("MATCH (c:Chunk) RETURN count(c) as count")[0]['count']
        total_entities = graph.query("MATCH (e:__Entity__) RETURN count(e) as count")[0]['count']
        total_relationships = graph.query("MATCH ()-[r]->() RETURN count(r) as count")[0]['count']
        
        # 统计问答会话数
        try:
            total_sessions = graph.query("MATCH (s:Session) RETURN count(s) as count")[0]['count']
        except:
            total_sessions = 0
        ...
        logging.info(f"Statistics query successful: {result}")
        return create_api_response('Success', data=result)
        
    except Exception as e:
        error_message = str(e)
        logging.exception(f'Statistics query failed: {error_message}')
        return create_api_response('Failed', message='Statistics query failed', error=error_message)

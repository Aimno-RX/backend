...
# LangChain chat models - 优化版：只保留DeepSeek需要的ChatOpenAI
...
from src.custom_neo4j_retriever import CustomNeo4jRetriever
...
def get_total_tokens(ai_response, llm):
    """
    获取AI响应的token使用量（优化版：只支持DeepSeek/ChatOpenAI）
    """
    try:
...
def create_retriever(neo_db, document_names, chat_mode_settings, search_k, score_threshold, ef_ratio, embedding_function):
    """
    创建检索器 - 使用CustomNeo4jRetriever确保query_vector参数正确传递
    """
    search_kwargs = {
        'top_k': search_k,
    ...
    if document_names and chat_mode_settings.get("document_filter"):
        search_kwargs['filter'] = {'fileName': {'$in': document_names}}
        logging.info(f"Created retriever with document filter for: {document_names}")
    else:
        logging.info(f"Successfully created retriever with search_k={search_k}, score_threshold={score_threshold}")
    
    # 使用自定义检索器
    retriever = CustomNeo4jRetriever(
        neo4j_vector=neo_db,
...
def get_neo4j_retriever(graph, document_names,chat_mode_settings, score_threshold=CHAT_SEARCH_KWARG_SCORE_THRESHOLD):
    try:
        ...
        EMBEDDING_FUNCTION, _ = load_embedding_model(EMBEDDING_MODEL)
        retriever = create_retriever(neo_db, document_names, chat_mode_settings, search_k, score_threshold, ef_ratio, EMBEDDING_FUNCTION)
        return retriever
    except Exception as e:
        index_name = chat_mode_settings.get("index_name")

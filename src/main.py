...
def get_dynamic_processing_profile(file_size_bytes: int) -> dict:
    ...
    if file_size_kb <= 50:
        return {
            ...
            "chunk_overlap": 80,
            "update_graph_chunk_processed": 20,
            "chunks_to_combine": 1,
            "enable_triplet_enhancement": True,
        }
    elif file_size_kb <= 250:
        return {
            "token_chunk_size": 900,
            "chunk_overlap": 80,
            "update_graph_chunk_processed": 30,
            "chunks_to_combine": 2,
            "enable_triplet_enhancement": True,
        }
        ...
        return {
            ...
            "chunk_overlap": 80,
            "update_graph_chunk_processed": 40,
            "chunks_to_combine": 2,
            "enable_triplet_enhancement": True,
        }
    elif file_size_kb <= 1000:
        return {
            "token_chunk_size": 1100,
            "chunk_overlap": 60,
            "update_graph_chunk_processed": 50,
            "chunks_to_combine": 2,
            "enable_triplet_enhancement": True,
        }
        ...
        return {
            "token_chunk_size": 1000,
            "chunk_overlap": 60,
            "update_graph_chunk_processed": 60,
            "chunks_to_combine": 2,
            "enable_triplet_enhancement": True,
        }
    elif file_size_kb <= 20000:
        return {
            "token_chunk_size": 900,
            "chunk_overlap": 50,
            "update_graph_chunk_processed": 60,
            "chunks_to_combine": 2,
            "enable_triplet_enhancement": False,
        }
    else:
        return {
...
async def processing_source(credentials, params, pages, merged_file_path=None, is_uploaded_from_local=None):
  ...
  file_size_bytes = 0
  try:
    param_file_size = getattr(params, "file_size", None)
    if param_file_size not in [None, ""]:
      file_size_bytes = int(param_file_size)
    elif merged_file_path and os.path.exists(merged_file_path):
      file_size_bytes = os.path.getsize(merged_file_path)
    else:
      result_for_file_size = graphDb_data_Access.get_current_status_document_node(params.file_name)
      if len(result_for_file_size) > 0:
        db_file_size = result_for_file_size[0].get("fileSize") or result_for_file_size[0].get("file_size")
        if db_file_size not in [None, ""]:
          file_size_bytes = int(db_file_size)
  except Exception as file_size_error:
    logging.warning(f"Unable to read file size for dynamic profile: {file_size_error}")

  logging.info(
    f"Dynamic profile file size check for {params.file_name}: "
    f"merged_file_path={merged_file_path}, file_size_bytes={file_size_bytes}"
  )

  if file_size_bytes > 0:
    profile = get_dynamic_processing_profile(file_size_bytes)
  else:
    logging.warning(
      f"Dynamic profile fallback used for {params.file_name} because file_size_bytes={file_size_bytes}"
    )
    profile = {
      "token_chunk_size": params.token_chunk_size or 1000,
  ...
  params.token_chunk_size = profile["token_chunk_size"]
  params.chunk_overlap = profile["chunk_overlap"]
  params.chunks_to_combine = profile["chunks_to_combine"]

  logging.info(
    f"Dynamic processing profile applied for {params.file_name}: "
    f"token_chunk_size={params.token_chunk_size}, "
    f"chunk_overlap={params.chunk_overlap}, "
    f"update_graph_chunk_processed={profile['update_graph_chunk_processed']}, "
    f"chunks_to_combine={params.chunks_to_combine}, "
    f"enable_triplet_enhancement={profile['enable_triplet_enhancement']}"
  )
  ...
  try:
    link_result = graph.query(
      ...
      WITH img, c, ABS(c.position - img.paragraphIndex) AS dist
      WHERE dist <= 5
      WITH img, c, dist
      ORDER BY dist ASC
  ...
  if len(result) > 0:
    ...
    if result[0]['Status'] != 'Processing':      
      ...
      update_graph_chunk_processed = profile["update_graph_chunk_processed"]
      # selected_chunks = []
      ...
      for i in range(0, len(chunkId_chunkDoc_list), update_graph_chunk_processed):
        ...
        if bool(is_cancelled_status) == True:
          ...
          node_count,rel_count,latency_processed_chunk,token_usage = await processing_chunks(
              ...
              profile["enable_triplet_enhancement"]
          )
      ...
      if is_uploaded_from_local and bool(is_cancelled_status) == False:
        if GCS_FILE_CACHE:
          ...
          logging.info(f'Keep local merged file for future regenerate/re-upload flow: {merged_file_path}')  
...
#create vector index and update chunk node with embedding
  ...
  start_triplet_enhancement = time.time()
  if enable_triplet_enhancement:
    try:
      enhancement_pipeline = TripletEnhancementPipeline()
      graph_documents = enhancement_pipeline.enhance_graph_documents_with_sentence_rules(
      ...
      logging.info(
        "Triplet enhancement stats - llm: %s, rule: %s, merged: %s, deduplicated: %s",
        enhancement_pipeline.statistics.get("llm_triplets", 0),
        enhancement_pipeline.statistics.get("rule_triplets", 0),
        enhancement_pipeline.statistics.get("merged_triplets", 0),
        enhancement_pipeline.statistics.get("deduplicated_triplets", 0),
      )
    except Exception as enhancement_error:
      logging.warning(f'Triplet enhancement skipped due to error: {enhancement_error}')
  else:
    logging.info(f"Triplet enhancement disabled for large file: {file_name}")

  end_triplet_enhancement = time.time()
  latency_processing_chunk["triplet_enhancement"] = f'{(end_triplet_enhancement - start_triplet_enhancement):.2f}'
  start_save_token = time.time()
  if get_value_from_env("TRACK_TOKEN_USAGE", "false", "bool"):
  ...
  elapsed_optimize = end_optimize - start_optimize
  logging.info(f'Time taken to optimize entities: {elapsed_optimize:.2f} seconds')
  latency_processing_chunk["entity_optimization"] = f'{elapsed_optimize:.2f}'
  # ================== 优化结束 ==================
...
def get_source_list_from_graph(credentials):
  ...
  # Close connection after getting the result
...
def failed_file_process(uri,file_name, merged_file_path):
  ...
  if GCS_FILE_CACHE:
      ...
      logging.info(f'Keep local failed file for retry/re-upload flow: {merged_file_path}')
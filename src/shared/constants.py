...
// Collect images linked to chunks via ILLUSTRATES
WITH d, avg_score, chunks, rels, nodes, entities
UNWIND chunks AS chunkScore
WITH d, avg_score, chunks, rels, nodes, entities, chunkScore.chunk AS chunkNode
OPTIONAL MATCH (img:ExerciseImage)-[:ILLUSTRATES]->(chunkNode)
WITH d, avg_score, chunks, rels, nodes, entities, collect(DISTINCT img) AS images
...
WITH d, avg_score,
    ...
    [img IN images WHERE img IS NOT NULL |
        {imageUrl: img.imageUrl, exerciseName: img.exerciseName, description: img.description,
         startingPosture: img.startingPosture, movementDescription: img.movementDescription,
         targetBodyParts: img.targetBodyParts, precautions: img.precautions}
    ] AS imageDetails,
    entities
// Combine texts into response text
WITH d, avg_score, chunkdetails, entityIds, relIds, imageDetails,
    "Text Content:\n" + apoc.text.join(texts, "\n----\n") +
    "\n----\nEntities:\n" + apoc.text.join(nodeTexts, "\n") +
    "\n----\nRelationships:\n" + apoc.text.join(relTexts, "\n") +
...
ADDITIONAL_INSTRUCTIONS = """You are a medical knowledge graph extraction system for Chinese medical texts.

STRICT RULES:
1. Only extract entities whose types are explicitly listed in allowed_nodes.
2. Only extract relationships that exactly match allowed_relationships.
3. Do NOT invent entity types or relationship types not in the schema.
4. Entity TYPE must exactly match one of the allowed_nodes strings (could be Chinese or English).
5. Entity NAME (id) must use the original Chinese medical terms from the text.
6. If a disease is mentioned, always try to link it to symptoms, anatomy, causes, or treatments if explicitly stated.
7. For Chinese medical text: use Chinese entity names but match entity TYPES exactly to allowed_nodes list.
8. If no valid relationship exists, return an empty graph instead of guessing.

Output must strictly follow the graph schema."""

...
def get_graph_results(credentials, document_names):
    ...
    try:
        ...
        # 解析document_names
        if document_names:
            doc_names_list = list(map(str, json.loads(document_names)))
        else:
            doc_names_list = []
        
        # 如果document_names为空，查询所有文档
        if not doc_names_list or len(doc_names_list) == 0:
            # 获取所有已完成的文档
            all_docs = get_completed_documents(driver)
            if all_docs:
                doc_names_list = all_docs[:10]  # 限制最多10个文档
                logging.info(f"No document names provided, querying first {len(doc_names_list)} documents: {doc_names_list}")
            else:
                logging.warning("No completed documents found in database")
                return {"nodes": [], "relationships": []}
        
        query = GRAPH_QUERY.format(graph_chunk_limit=GRAPH_CHUNK_LIMIT)
        records, summary , keys = execute_query(driver, query.strip(), doc_names_list)
